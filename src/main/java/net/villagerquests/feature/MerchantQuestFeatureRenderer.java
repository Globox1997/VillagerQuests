...
// public class MerchantQuestFeatureRenderer<T extends LivingEntity, M extends EntityModel<T>> extends FeatureRenderer<T, M> {
//     private final TextRenderer textRenderer;
//     private final ModelPart modelPart;

//     public MerchantQuestFeatureRenderer(FeatureRendererContext<T, M> featureRendererContext, TextRenderer textRenderer, ModelPart modelPart) {
...
@Environment(EnvType.CLIENT)
public class MerchantQuestFeatureRenderer<T extends LivingEntity, M extends EntityModel<T>> extends FeatureRenderer<T, M> {
    private static final Identifier TEXTURE = new Identifier("villagerquests:textures/entity/quest.png");
    private final QuestEntityModel<MerchantEntity> model;
    private final EntityRenderDispatcher dispatcher;

    public MerchantQuestFeatureRenderer(FeatureRendererContext<T, M> context, EntityModelLoader loader, EntityRenderDispatcher dispatcher) {
        super(context);
        this.model = new QuestEntityModel<>(loader.getModelPart(VillagerQuestsClient.QUEST_LAYER));
        this.dispatcher = dispatcher;
    }

    @Override
    public void render(MatrixStack matrices, VertexConsumerProvider vertexConsumersProvider, int light, T entity, float limbAngle, float limbDistance, float tickDelta, float animationProgress,
            float headYaw, float headPitch) {
        VertexConsumer vertexConsumer = vertexConsumersProvider.getBuffer(RenderLayer.getEntityCutoutNoCull(TEXTURE));
        matrices.multiply(Vec3f.NEGATIVE_Y.getRadialQuaternion(this.dispatcher.camera.getRotation().getW()));
        this.model.render(matrices, vertexConsumer, 15728880, OverlayTexture.DEFAULT_UV, 1.0F, 1.0F, 1.0F, 1.0F);
        this.model.setAngles((MerchantEntity) entity, limbAngle, limbDistance, animationProgress, headYaw, headPitch);
    }
